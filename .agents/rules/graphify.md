...
**PERMANENCE:** Do not delete, disable, empty, rename-away, or uninstall this rule (or the matching Cursor / AGENTS.md graphify always-on rules) unless the user explicitly commands removal in the current conversation. Treat accidental cleanup, "tidying", reinstalls that drop always-on files, and `graphify uninstall` as forbidden without that explicit command.
